...
const UploadForm = ({ progress }) => {
  ...
  const upload = async (e) => {
    ...
    if (receiverEmail.length !== 0 && !receiversEmailSchema.safeParse(receiverEmail).success) {
    ...
    try {
      ...
      const res = await axios.post("http://secure-share-server.vercel.app", formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
      toast.success("File successfully uploaded, file id copied to clipboard");
      //copy file id to clipboard
      navigator.clipboard.writeText(res.data.fileId);
    } catch (error) {
      console.log(error);
    ...
    <div className="text-center">
      ...
      <div className="my-6 flex flex-col items-start">
        ...
        <input
          required={true}
          type="email"
          placeholder="Enter Receiver's Email Adress (optional)"
          value={receiverEmail}
